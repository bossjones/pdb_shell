...
class Pdb(pdb.Pdb, ConfigurableClass):
    ...
    def parseline(self, line):
        ...
        if cmd and hasattr(self, 'do_'+cmd) and (cmd in self.curframe.f_globals or
            ...
            line = '!' + line
            return pdb.Pdb.parseline(self, line)
        if cmd and (not hasattr(self, 'do_'+cmd) or self.curframe.f_locals.get('_pdb_shell', False)):
            history = self.curframe.f_locals.get('_pdb_history', [])
            history.append(line)
            self.curframe.f_locals['_pdb_history'] = history
            line = '!' + line
            return pdb.Pdb.parseline(self, line)
